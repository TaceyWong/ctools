...
func init() {
	// Background
	img, _, err := image.Decode(bytes.NewReader(Ambericer_jpg))
	if err != nil {
		panic(err)
	}
	...
	// Windows: Next
	x, y = nextWindowLabelPosition()
	drawTextWithShadow(imageWindows, "下一个", x, y, 1, fontColor)
	x, y = nextWindowPosition()
	drawWindow(imageWindows, x, y, 5*blockWidth, 5*blockHeight)

	// Windows: Score
	x, y = scoreTextBoxPosition()
	drawTextBox(imageWindows, "分数", x, y, textBoxWidth())

	// Windows: Level
	x, y = levelTextBoxPosition()
	drawTextBox(imageWindows, "级别", x, y, textBoxWidth())

	// Windows: Lines
	x, y = linesTextBoxPosition()
	drawTextBox(imageWindows, "行数", x, y, textBoxWidth())

	// Gameover
	imageGameover.Fill(color.NRGBA{0x00, 0x00, 0x00, 0x80})
	y = (ScreenHeight - blockHeight) / 2
	drawTextWithShadowCenter(imageGameover, "游戏结束\n\n按空格键开始", 0, y, 4, color.White, ScreenWidth)
}
